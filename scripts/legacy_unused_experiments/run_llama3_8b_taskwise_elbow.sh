...
export MODEL_TAG=${MODEL_TAG:-llama3_8b_layerwise_elbow}
export METHODS=${METHODS:-hilora lora pissa adalora dora}
...
if [[ -z "${HILORA_EXTRA_ARGS:-}" ]]; then
  export HILORA_EXTRA_ARGS="--hilora_taskwise_elbow --hilora_allocate_full_rank --hilora_task_elbow_energy_threshold 0.0 --hilora_task_min_active_rank 8 --hilora_task_bonus_map sst2:2,mrpc:2,qnli:3,rte:3,qqp:2,mnli:4 --hilora_layer_bonus_map q_proj:0,v_proj:1 --hilora_depth_bonus_map early:0,mid:1,late:2"
fi
export LOG_DIR=${LOG_DIR:-logs/llama3_8b_layerwise_elbow}
mkdir -p "$LOG_DIR"
